...
WITH Filtered_Retail AS (
    SELECT 
        ...
        Country
    FROM Online_Retail
    WHERE CustomerID IS NOT NULL --filter out the CustomerID without value
),

Valid_Transactions AS (
    SELECT *
    FROM Filtered_Retail
    WHERE Quantity > 0 AND UnitPrice > 0 --Retain entries with valid Quantity and unitprice
),

Duplicate_Transactions AS (
    SELECT *,
           ROW_NUMBER() OVER (PARTITION BY InvoiceNo, StockCode, Quantity ORDER BY InvoiceDate) AS row_num
    FROM Valid_Transactions --compute the number of times an entry occurs in the table
)
